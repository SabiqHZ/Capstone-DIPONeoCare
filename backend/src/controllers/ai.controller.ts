...
function parseVision(body: unknown): VisionResultPayload | null {
  ...
  if (
    ...
    return null;
  if (
    !isFlag(value.activity.sleeping) ||
    !isFlag(value.activity.awake) ||
    Number(value.activity.sleeping) + Number(value.activity.awake) !== 1 ||
    !isFlag(value.visualCrying.detected) ||
    !isFlag(value.anomaly.detected) ||
    !validConfidence(value.activity.confidence) ||
    !validConfidence(value.visualCrying.confidence) ||
    !validConfidence(value.anomaly.confidence)
  )
    return null;
  if (
    value.anomaly.detected &&
    !["bantal", "guling", "mainan"].includes(value.anomaly.type ?? "")
  )
    return null;
...
function parseAudioResults(
  body: any,
): { macAddress: string; results: AudioSecondResult[] } | null {
  if (
    !body ||
...
async function emitCryingAlerts(
  ...
  for (const status of statuses.filter(
    (item) =>
      item.flags.crying &&
      item.cryingDurationSec >= (device.crying_min_duration_sec ?? 10),
  )) {
    const alert = await inferenceService.createCryingAlertIfDue(
      device,
      status.cryingDurationSec,
    );
    if (alert) {
      broadcastAlert(io, {
        id: alert.id,
  ...
  async receiveVisionResult(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!payload) {
        res
          .status(400)
          .json({ success: false, error: "Payload vision tidak valid" });
        return;
      }
      ...
      const finalized = await inferenceService.finalizeMatches(device, [
        payload.captureId,
      ]);
      ...
      if (anomalyAlert) {
        broadcastAlert(io, {
          ...
          timestamp: anomalyAlert.created_at,
          acknowledged: false,
        });
      }
      res.json({
  ...
  async receiveAudioResults(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!payload) {
        res.status(400).json({
          success: false,
          error: "Audio harus berisi tepat lima hasil per detik yang valid",
        });
        return;
      }
      const device = await deviceService.getDeviceByMac(payload.macAddress);
      ...
      const finalized = await inferenceService.finalizeMatches(
        device,
        payload.results.map((item) => item.captureId),
      );
      emitFinalized(finalized);
      await emitCryingAlerts(device, finalized);
