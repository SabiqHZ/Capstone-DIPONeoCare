...
import { ActivityFlags, AudioSecondResult, DeviceContext, VisionResultPayload } from '../types';
...
export const inferenceService = {
  async saveVisionResult(device: DeviceContext, payload: VisionResultPayload) {
    ...
    const { error } = await supabaseAdmin.from('ai_vision_results').upsert({
      ...
      night_vision_active: payload.nightVision ?? false,
  ...
  async saveAudioResults(device: DeviceContext, results: AudioSecondResult[]) {
    ...
    const { error } = await supabaseAdmin.from('ai_audio_results').upsert(rows, {
      onConflict: 'device_id,capture_id',
    });
    if (error) throw new Error(error.message);
  },

  async finalizeMatches(device: DeviceContext, captureIds: string[]): Promise<FinalizedStatus[]> {
    if (!device.baby_id || captureIds.length === 0) return [];
    const [{ data: visions, error: visionError }, { data: audios, error: audioError }] = await Promise.all([
      supabaseAdmin.from('ai_vision_results').select('*').eq('device_id', device.id).in('capture_id', captureIds),
      supabaseAdmin.from('ai_audio_results').select('*').eq('device_id', device.id).in('capture_id', captureIds),
    ]);
    if (visionError) throw new Error(visionError.message);
    if (audioError) throw new Error(audioError.message);

    const audioByCapture = new Map((audios ?? []).map((audio: any) => [audio.capture_id, audio]));
    ...
    for (const vision of visions ?? []) {
      ...
      const flags: ActivityFlags = crying
        ...
        : { sleeping: Boolean(vision.sleeping), awake: Boolean(vision.awake), crying: false };

      const activity = flags.crying ? 'crying' : flags.sleeping ? 'sleeping' : 'awake';
      ...
      const cryingDurationSec = crying ? await this.getCurrentCryingDuration(device.baby_id, vision.captured_at) : 0;
      await deviceService.upsertBabyStatus(device.baby_id, {
        activity,
        flags,
        cryingDurationSec,
        alertLevel: crying ? 'warning' : 'normal',
        nightVisionActive: Boolean(vision.night_vision_active),
  ...
  async trackAnomaly(device: DeviceContext, payload: VisionResultPayload) {
    ...
    const { data: active, error } = await supabaseAdmin.from('anomaly_events')
      .select('*').eq('device_id', device.id).is('resolved_at', null).maybeSingle();
    if (error) throw new Error(error.message);

    if (!payload.anomaly.detected) {
      if (active) {
        const { error: resolveError } = await supabaseAdmin.from('anomaly_events')
          .update({ resolved_at: payload.timestamp, last_detected_at: payload.timestamp }).eq('id', active.id);
    ...
    if (!event || event.anomaly_type !== payload.anomaly.type) {
      if (event) {
        await supabaseAdmin.from('anomaly_events').update({ resolved_at: payload.timestamp }).eq('id', event.id);
      }
      const { data, error: insertError } = await supabaseAdmin.from('anomaly_events').insert({
        baby_id: device.baby_id, device_id: device.id, anomaly_type: payload.anomaly.type,
        started_at: payload.timestamp, last_detected_at: payload.timestamp,
      }).select().single();
      if (insertError) throw new Error(insertError.message);
      event = data;
    } else {
      const { data, error: updateError } = await supabaseAdmin.from('anomaly_events')
        .update({ last_detected_at: payload.timestamp }).eq('id', event.id).select().single();
      if (updateError) throw new Error(updateError.message);
      event = data;
    }

    const durationSeconds = (Date.parse(event.last_detected_at) - Date.parse(event.started_at)) / 1_000;
    if (durationSeconds >= 15 && !event.alert_sent_at) {
      ...
      if (alert) {
        await supabaseAdmin.from('anomaly_events').update({ alert_sent_at: new Date().toISOString() }).eq('id', event.id);
        return alert;
      }
    }
    return null;
  ...
  async getCurrentCryingDuration(babyId: string, recordedAt: string): Promise<number> {
    const { data, error } = await supabaseAdmin.from('baby_activity_samples')
      .select('crying, recorded_at').eq('baby_id', babyId).lte('recorded_at', recordedAt)
    ...
    for (const sample of data ?? []) {
      ...
      if (!sample.crying || (previous !== null && previous - time > 1_500)) break;
      seconds += 1;
      previous = time;
    }
    return seconds;
  ...
  async createCryingAlertIfDue(device: DeviceContext, durationSeconds: number) {
    if (!device.baby_id) return null;
    const cooldownStart = new Date(Date.now() - (device.notification_cooldown_sec ?? 120) * 1_000).toISOString();
    const { data: recent, error } = await supabaseAdmin.from('alert_logs').select('id')
      .eq('baby_id', device.baby_id).eq('type', 'CRYING_DETECTED').gte('created_at', cooldownStart).maybeSingle();
    if (error) throw new Error(error.message);
