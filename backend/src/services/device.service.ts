...
export const deviceService = {
  ...
  async verifyDeviceToken(macAddress: string, token: string): Promise<boolean> {
    ...
    return crypto.timingSafeEqual(
      Buffer.from(data.device_token_hash, "hex"),
  ...
  async updateConfig(
    deviceId: string,
    config: { cryingMinDurationSec?: number; notificationCooldownSec?: number },
  ) {
    const { data, error } = await supabaseAdmin
      .from("devices")
      .update({
        ...(config.cryingMinDurationSec !== undefined
          ? { crying_min_duration_sec: config.cryingMinDurationSec }
          : {}),
        ...(config.notificationCooldownSec !== undefined
          ? { notification_cooldown_sec: config.notificationCooldownSec }
          : {}),
      })
      .eq("id", deviceId)
      .select()
  ...
  async upsertBabyStatus(
    ...
    if (
      current?.updated_at &&
      Date.parse(current.updated_at) > Date.parse(status.updatedAt)
    )
      return;
    ...
    const { error } = await supabaseAdmin.from("baby_activity_samples").upsert(
      {
        ...
        sleeping: payload.flags.sleeping,
        awake: payload.flags.awake,
        crying: payload.flags.crying,
        visual_crying: payload.visualCrying,
