    ...
    >
      <View style={styles.cardThumb}>
        ...
        <View
          style={[
            styles.onlinePill,
            { backgroundColor: isOnline ? "#052e16" : Colors.borderLight },
          ]}
        >
          <View
            style={[
              styles.onlineDot,
              { backgroundColor: isOnline ? "#4ade80" : Colors.textDisabled },
            ]}
          />
          <Text
            style={[
              styles.onlinePillText,
              { color: isOnline ? "#4ade80" : Colors.textDisabled },
            ]}
          >
...
export default function NurseDashboard() {
  ...
  const { cryingCount, faceAnomalyCount, onlineCount } = useMemo(() => {
    ...
    babies.forEach((baby) => {
      ...
      const isCry = rtStatus
        ? rtStatus.isCrying
        : (baby.baby_statuses?.is_crying ?? false);
      const isFace = rtStatus
        ? rtStatus.faceAnomaly !== "none"
        : Boolean(
            baby.baby_statuses?.face_anomaly &&
            baby.baby_statuses.face_anomaly !== "none",
          );
      const isOn = rtStatus
        ? rtStatus.deviceOnline
        : (baby.devices?.is_online ?? false);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.summaryBar}>
        ...
        <View style={styles.summaryDivider} />
        <View style={styles.summaryItem}>
          <Text style={[styles.summaryNum, { color: "#3f384aff" }]}>
            {faceAnomalyCount}
          </Text>
      ...
      {isLoading ? (
        ...
        <View style={styles.centerBox}>
          ...
          <Text style={styles.emptyText}>
            Tap tombol + untuk mendaftarkan bayi baru
          </Text>
          <TouchableOpacity
